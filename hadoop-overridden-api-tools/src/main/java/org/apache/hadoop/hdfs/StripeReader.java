...
abstract class StripeReader {
  ...
  boolean readChunk(final LocatedBlock block, int chunkIndex)
    ...
    if (readerInfos[chunkIndex] == null) {
      ...
      List<ExtendedBlock> badBlocks = dfsStripedInputStream.getBadBlocks();
      boolean isInBadBlocks = false;
      if (badBlocks != null) {
        for (int i = 0; i < badBlocks.size(); i++) {
          ExtendedBlock blk = badBlocks.get(i);
          if (blk.getBlockPoolId() == null) {
            if (blk.getLocalBlock().equals(block.getBlock().getLocalBlock())) {
              isInBadBlocks = true;
            }
          } else {
            if (blk.equals(block))
